...
fn default_hook(info: &PanicInfo<'_>) {
    ...
    let backtrace = BacktraceStyle::full();
    ...
    let write = |err: &mut dyn crate::io::Write| {
        let _ = writeln!(err, "panicked at '{msg}', {location}");
...
pub fn begin_panic_handler(info: &PanicInfo<'_>) -> ! {
    let _ = core::writeln!(StderrPrinter,"'{info}'");
...
fn do_panic(msg: Box<dyn Any + Send>) -> ! {
    ...
    let _ = core::writeln!(StderrPrinter,"failed to initiate panic, error {}", code.0);
...
fn stack_trace() {
    ...
    extern "C" fn callback(
        ...
        data.counter += 1;
        let _ = core::writeln!(StderrPrinter,
            "{:4}:{:#19x} - <unknown>",
            data.counter,
...
pub const fn begin_panic<M: Any + Send>(msg: M) -> ! {
    unwinding::panic::begin_panic(Box::new(msg));
    crate::sys::abort_internal();
}
...
pub fn rust_panic_without_hook(payload: Box<dyn Any + Send>) -> ! {
    unwinding::panic::begin_panic(Box::new(payload));
    crate::sys::abort_internal();
}

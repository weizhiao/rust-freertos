...
pub unsafe fn panic(data: Box<dyn Any + Send>) -> u32 {
    ...
    let exception_param = &mut exception._uwe as &mut uw::UnwindException;
    return uw::_Unwind_RaiseException(exception_param).0 as u32;

    extern "C" fn exception_cleanup(
        _unwind_code: uw::UnwindReasonCode,
        exception: *mut uw::UnwindException,
    ) {
        unsafe {
...
pub unsafe fn cleanup(ptr: *mut u8) -> Box<dyn Any + Send> {
    let exception = ptr as *mut uw::UnwindException;
